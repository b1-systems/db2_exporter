...
linux:
	@echo build linux
	@mkdir -p ./dist/ibmdb2_exporter.$(VERSION).linux-amd64
	@PKG_CONFIG_PATH=${PWD} GOOS=linux go build $(GOFLAGS) -o ./dist/ibmdb2_exporter.$(VERSION).linux-amd64/ibmdb2_exporter
	@cp default-metrics.toml ./dist/ibmdb2_exporter.$(VERSION).linux-amd64
	@(cd dist ; tar cfz ibmdb2_exporter.$(VERSION).linux-amd64.tar.gz ibmdb2_exporter.$(VERSION).linux-amd64)
